...
def _prep_training_dataset(ds, batch_size, mode='region', upsample_factor=None, shuffle=True):
    """
    ...
    if mode == 'region':
        ...
        subj_ind = np.array(subj_ind, dtype=np.int32)
        yobs = np.array(yobs, dtype=np.float32)
        iext = np.array(iext, dtype=np.float32)
        iext_upsampled = np.array(iext_upsampled, dtype=np.float32)
        ...
        for i in range(nsub):
            yaug = np.moveaxis(np.concatenate([ds.y[i,:,:], iext[i,:,:]], axis=1), [1,2], [2,1])
            isubs.append(i)
            yaugs.append(yaug)
...
class History:
    def __init__(self, model):
        ...
        self.names = ["epoch", "loss", "betax", "betap"] + model_names
        self.fmts = ["%6d", "%14.2f", "%6.3f", "%6.3f"] + model_fmts
        self._hist = []
    ...
    def add(self, epoch, loss, betax, betap, model):
        model_values = model.tracked_variables_values()
        self._hist.append([epoch, loss, betax, betap] + model_values)
...
def train(model, dataset, runner, fh=None, callback=None):

    try:
        upsample_factor = model.upsample_factor
    except AttributeError:
        upsample_factor = None
    training_dataset = _prep_training_dataset(dataset, runner.batch_size, model.training_mode, upsample_factor)
    ...
    for epoch in range(runner.epochs):
        ...
        hist.add(epoch, loss.result().numpy(), betax.numpy(), betap.numpy(), model)
